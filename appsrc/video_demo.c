...
void DemoRun()
{
	...
	u32 abc_display_base = 0x44a40000;
	...
	while (userInput != 'q')
	{
		...
		case '7':
			VideoStop(&videoCapt);
//			ABC_DISPLAY_IP_mWriteReg(abc_display_base, 0, 0x00FFFFFF);
			ABC_DISPLAY_IP_mWriteReg(abc_display_base, 0, 0x00FF0000);
			VideoStart(&videoCapt);
//			nextFrame = videoCapt.curFrame + 1;
//			if (nextFrame >= DISPLAY_NUM_FRAMES)
//			{
...
//			DisplayChangeFrame(&dispCtrl, nextFrame);
		...
		case '8':
			VideoStop(&videoCapt);
			ABC_DISPLAY_IP_mWriteReg(abc_display_base, 0, 0x00000000);
			VideoStart(&videoCapt);
//			nextFrame = videoCapt.curFrame + 1;
//			nextFrame = videoCapt.curFrame + 1;
//			if (nextFrame >= DISPLAY_NUM_FRAMES)
...
void DemoPrintMenu()
{
	...
	xil_printf("7 - Test reg with Fs\n\r");
	xil_printf("8 - Test reg with 0s\n\r");
